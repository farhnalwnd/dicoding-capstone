    ...
    if api_key:
        try:
            prompt = _build_gemini_prompt(
                match_score, matched_skills, missing_skills, recommendation, job_description
            )
...
def _build_learning_plan(missing_skills: List[str]) -> List[Dict]:
    ...
    if len(missing_skills) > 2:
        plan.append({"duration": "Week 3-4", "tasks": [
    ...
    # Week 5
    plan.append({"duration": "Week 5", "tasks": [
...
def _build_career_recommendations(matched_skills: List[str]) -> Dict:
    ...
    suitability = ("Your profile is aligned with modern technical roles, but acquiring additional "
    ...
    if matched_skills:
        ...
        if any(s in ML_SKILLS for s in lowered):
            suitability = ("You possess strong machine learning and data science fundamentals. "
            ...
            suitability = ("Your skills point toward interactive user interfaces. Adding backend "
...
def generate_mock_recommendations(
    ...
    improvement = min(MAX_IMPROVEMENT, max(MIN_IMPROVEMENT, (100.0 - match_score) * IMPROVEMENT_FACTOR))
...
def generate_advisor_pdf(rec: Dict) -> bytes:
    ...
    page.insert_text((left_margin, y), "1. Analysis Overview", fontsize=12, fontname="helv", color=(0.02, 0.35, 0.52))
    y += 20
    ...
    # Section 2: Skills
    page.draw_rect(fitz.Rect(left_margin, y - 12, right_margin, y + 5), color=(0.9, 0.9, 0.9), fill=(0.9, 0.9, 0.9))
    page.insert_text((left_margin, y), "2. Skills Assessment", fontsize=12, fontname="helv", color=(0.02, 0.35, 0.52))
    y += 20
    ...
    for plan in rec.get("learning_plan", []):
        add_page_if_needed(y, 60)
        y = draw_text_wrapped(f"- {plan.get('duration', 'Phase')}:", left_margin, y, 10, "helv")
